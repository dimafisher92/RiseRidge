...
// Renders a markdown string with the same visual treatment the blog previously
// got from TinaRichText, so switching the data source keeps articles identical.
const components = {
  ...
  a: ({ children, href }: { children?: React.ReactNode; href?: string }) => (
    <a href={href} className="text-electric hover:text-signal underline underline-offset-2 transition-colors">
      {children}
    </a>
  ),
  ...
  em: ({ children }: { children?: React.ReactNode }) => <em className="italic text-muted/90">{children}</em>,
  hr: () => <hr className="my-10 border-border" />,
  img: ({ src, alt }: { src?: string; alt?: string }) => (
    // eslint-disable-next-line @next/next/no-img-element
    <img src={src} alt={alt ?? ''} className="my-8 w-full rounded-[6px] border border-border" />
  ),
  // Inline vs fenced code — render the block form ourselves so the inner code
  // doesn't pick up the inline pill styling.

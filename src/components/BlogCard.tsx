...
export function BlogCard({ slug, title, excerpt, date, category, authorName, coverImage, index }: BlogCardProps) {
  return (
    <motion.article
      ...
      className="flex flex-col overflow-hidden rounded-[6px] border border-line bg-white shadow-[0_10px_34px_rgba(21,20,15,0.05)] hover:-translate-y-1 hover:shadow-[0_16px_44px_rgba(21,20,15,0.10)] transition-all duration-[400ms] ease-[cubic-bezier(.2,.7,.2,1)] group"
    >
      {coverImage && (
        <Link href={`/blog/${slug}`} aria-hidden="true" tabIndex={-1} className="block aspect-[16/9] w-full overflow-hidden bg-panel">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={coverImage}
            alt=""
            className="h-full w-full object-cover transition-transform duration-[400ms] ease-[cubic-bezier(.2,.7,.2,1)] group-hover:scale-105"
          />
        </Link>
      )}
